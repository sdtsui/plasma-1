...
import (
	...
	"github.com/kyokan/minimum-viable-plasma/eth"
	"log"
	"math/big"
	"math/rand"
)
...
func (sink *TransactionSink) AcceptTransactionRequests(chch <-chan chan TransactionRequest) {
	go func() {
		for {
			ch := <-chch
			req := <-ch
			balance, err := sink.db.AddressDao.GetBalance(req.From)
			...
			if len(utxoTxs) == 1 {
				...
				totalAmount := utxoTxs[0].GetUTXO(&req.From).Amount
				...
				input1 = &chain.Input{
					...
					OutIdx: utxoTxs[1].GetUTXOIndex(&req.From),
				}

				totalAmount := big.NewInt(0)
				totalAmount = totalAmount.Add(utxoTxs[0].GetUTXO(&req.From).Amount, utxoTxs[1].GetUTXO(&req.From).Amount)
			...
			tx := chain.Transaction{
				Input0: &chain.Input{
					...
					OutIdx: utxoTxs[0].GetUTXOIndex(&req.From),
				},
				Input1: input1,
...
func (sink *TransactionSink) VerifyTransaction(tx *chain.Transaction) (bool, error) {
	...
	sig1Addr, err := crypto.Ecrecover(tx.SignatureHash(), tx.Sig0)

	if err != nil {
		return false, err
	}

	sig2Addr, err := crypto.Ecrecover(tx.SignatureHash(), tx.Sig1)

	if !bytes.Equal(prevOutput1.NewOwner.Bytes(), sig1Addr) {
		return false, errors.New("input 1 signature is not valid")
	}

	if !bytes.Equal(prevOutput2.NewOwner.Bytes(), sig2Addr) {
...
func (sink *TransactionSink) FindBestUTXOs(addr common.Address, amount *big.Int) ([]chain.Transaction, error) {
	txs, err := sink.db.AddressDao.GetTransactionsWithUTXOs(addr)
	...
	for i, ltx := range txs {
		for _, rtx := range txs[i:] {
			total := big.NewInt(0)
			total = total.Add(ltx.GetUTXO(&addr).Amount, rtx.GetUTXO(&addr).Amount)
	...
	for i := 0; i < 1000; i++ {
		...
		total := big.NewInt(0).Add(ltx.GetUTXO(&addr).Amount, rtx.GetUTXO(&addr).Amount)
